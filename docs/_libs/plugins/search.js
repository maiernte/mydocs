(function () {
  ...
  function saveData(maxAge, expireKey, indexKey) {
    ...
    addToDB('indexkey', expireKey, Date.now() + maxAge)
    addToDB('indexkey', indexKey, JSON.stringify(INDEXS))
    console.log(`saveData : expireKey=${expireKey}, indexKey=${indexKey}`)
  }
  ...
  async function init(config, vm) {
    ...
    var isExpired = (await readFromDB('indexkey', expireKey)) < Date.now();

    //INDEXS = JSON.parse(localStorage.getItem(indexKey));
    INDEXS = JSON.parse(await readFromDB('indexkey', indexKey))
